...
//Helpers
   ...
   initMutex(htable* ht){
	for (int i = 0; i < ht->size; i++)
		pthread_mutex_destroy(ht->mutex + i);
   	for(int i = 0; i < ht->size; i++)
        	pthread_mutex_init((ht->mutex) + i, NULL);
}
//End: Helpers
...
htable_init(htable *ht, int sz, int allow_resize) {
    ...
    ht->mutex = (pthread_mutex_t*) malloc(sizeof(pthread_mutex_t) * (ht->size));
	initMutex(ht);
	ht->rwl = (rwl*) malloc(sizeof(rwl));
	ht->rwl->mutex = (pthread_mutex_t*) malloc(sizeof(pthread_mutex_t));
	pthread_mutex_init(ht->rwl->mutex,NULL);
	rwl_init(ht->rwl);
}
...
htable_resize(htable *ht) {
	rwl_wlock(ht->rwl, 110);
	printf("Writer: resize | Grabs RW Lock\n");
	...
	free(ht->store);
	ht->store = new_store;
	printf("Writer: resize | Releases RW Lock\n");
	rwl_wunlock(ht->rwl);
}
...
htable_insert(htable *ht, char *key, void *val) {
	rwl_rlock(ht->rwl, 110);
	//printf("Reader: insert | Grabs RW Lock\n");
	int hcode = hashcode(key);	
	//this key/value tuple corresponds to slot "slot"
    int slot = hcode % ht->size;
	pthread_mutex_t* target = ht->mutex + slot;
	...
	while (curr) {
		if ((curr->hashcode == hcode ) && (strcmp(curr->key, key) == 0)) {
			...
			rwl_runlock(ht->rwl);
			//printf("Reader: insert | Realeases RW Lock\n");
...
htable_lookup(htable *ht, char *key) {
	rwl_rlock(ht->rwl, 110);
	//printf("Reader: look up | Grabs RW Lock\n");
	int hcode = hashcode(key);
    int slot = hcode % ht->size;
    pthread_mutex_t* target = ht->mutex + slot;
    pthread_mutex_lock(target);
	...
	pthread_mutex_unlock(target);
	//printf("Reader: look up | Releases RW Lock\n");
	rwl_runlock(ht->rwl);
	return NULL;
}
